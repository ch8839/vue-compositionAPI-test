...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const specificEmits = {
      ['update:sortOrder']: (v: any) => { TableHook.handleUpdateOrder(v) },
    ...
    const shouldUpdateHeight = computed<boolean>(() => (
      !!TableHook.m_height.value ||
      !!TableHook.m_maxHeight.value ||
    ...
    const bodyHeight = computed(() => {
      if (TableHook.m_height.value) {
        return {
          height: state.layout.bodyHeight ? state.layout.bodyHeight + 'px' : '',
        }
      } else if (TableHook.m_maxHeight.value) {
        const propsMaxHeight = Number(TableHook.m_maxHeight.value)
        return {
          'max-height':
            (props.showHeader
              ? propsMaxHeight -
    ...
    const fixedHeight = computed(() => {
      if (TableHook.m_maxHeight.value) {
    ...
    watch(TableHook.m_height, (v) => {
      v && state.layout.setHeight(v)
    }, { immediate: true })
    watch(TableHook.m_maxHeight, (v) => {
      v && state.layout.setMaxHeight(v)
    }, { immediate: true })
    watch(() => props.data, (v) => {
      state.store.setData(v)
    }, { immediate: true })
    watch(TableHook.m_expandRowKeys, (v) => {
    ...
    syncStatesWatch(TableHook.m_sortOrder, 'sortOrder')
    syncStatesWatch(TableHook.m_treeFieldNames, 'treeFieldNames')
    syncStatesWatch(TableHook.m_loadedKeys, 'loadedKeys')
    ...
    function resizeListener() {
      ...
      if ((TableHook.m_height.value || shouldUpdateHeight.value) && oldHeight !== height) {
        shouldUpdateLayout = true
      }
  ...
  render() {
    const {
      ...
      showSummary, data, summaryMethod, sumText, resizeProxyVisible, m_tooltipProps, tooltipClass,
    ...
    // todo🤡 : vue3 中有个奇怪的bug
    // 如果是 empty ? () => <> : undefined,哪怕这边改变了，body里面的empty仍然不是undefined
    const tableBodySlots = isEmpty ? {
      empty: () => <div
      ...
      >
        <span class={`${prefix}-empty-text`}>
          {getSlotsInRender(this, 'empty') || emptyText}
        </span>
      </div>,
    } : {}
    ...
    return <div
      class={{
        ...
        [`${prefix}-fluid-height`]: m_maxHeight,
    ...
    >
      ...
      {/* 表格的身体-前置插槽 */}
      {getSlotsInRender(this, 'preposition')}
      ...
      { <table-body
        ...
        v-slots={tableBodySlots}
        {...vSlots(tableBodySlots)}
      />
      }

      {loading && 
        <div class={`${prefix}-loading-block`}>
          {getSlotsInRender(this, 'loading') || <mtd-loading message={loadingMessage} />}
        </div>
      }

      {/* 表格的身体-后置插槽 */}
      {getSlotsInRender(this, 'postposition')}
