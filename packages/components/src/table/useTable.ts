import { computed, reactive, watch, hasProp } from '@ss/mtd-adapter'
...
export const useTable = (props: TableProps, ctx: any) => {
  ...
  // @Computed
  const m_sortOrder = computed(() => state.isSortControlled ? props.sortOrder : state.sortOrder)
  const m_loadedKeys = computed(() => state.isLoadedKeyControlled ? props.loadedKeys : state.loadedKeys)
  const m_expandRowKeys = computed(() => state.isExpandedControlled ? props.expandRowKeys : state.expandRowKeys)
  ...
  function reduceTreeData(data: any[], expandRowKeys: string[]) {
    ...
    return data.reduce((keys, item) => {
      if (item[children] && item[children].length && props.rowKey) {
        const key = getRowIdentity(item, props.rowKey as any)
        keys.push(key)
  ...
  return {
    m_treeFieldNames: TREE_FIELD_NAMES,
