...
:data="tableData"
    ...
    <mtd-table-column
      ...
      <template #filter-icon="scope">
        <i class="mtdicon mtdicon-down-thick" /></template>
      <template #filter-dropdown="scope">
        <div style="padding: 8px;">
          <mtd-input 
            style="margin-bottom: 8px"
            :model-value="scope.selectedValue[0]"
            @input="(v) => scope.setSelected([v])"
            @keyup.enter="scope.confirm"
            clearable
            placeholder="Search name" />
          <div>
            <mtd-button type="primary" @click="scope.confirm">查找</mtd-button>
          </div>
        </div>
      </template>
    </mtd-table-column>
