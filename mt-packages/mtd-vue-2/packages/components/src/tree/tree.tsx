import {
  defineComponent, PropType, markRaw, computed,
  reactive, toRefs, provide, Fragment, vueInstance,
  getSlotsInRender,
} from '@ss/mtd-adapter'
...
export default defineComponent({
  ...
  methods: {
    ...
    handleDragOver(event: DragEvent, treeNode: TreeNodeInterface) {
      ...
      const targetPosition = dropNode.$el
        .querySelector(`.${this.prefix}-node-content`)
    ...
    handleDragEnd(event: DragEvent) {
      ...
      if (draggingNode && dropNode) {
        ...
        const draggingParent = draggingNode.node.$parent
          ...
          : {
            data: { [this.m_fieldNames.children]: this.data },
            children: this.root,
          }
        ...
        if (dropType !== 'none') {
          removeChild(draggingParent, draggingNode.node, this.m_fieldNames)
        }
        if (dropType === 'before') {
          insertBefore(
            drogParent,
          ...
          insertChild(dropNode.node, draggingNodeCopy, -1, this.m_fieldNames)
        }
  ...
  render() {
    const { prefix, virtual, flatNodes, expandOnClickNode, draggable, root,
      data, emptyText, dragState, allHidden } = this
    ...
    /* {(!data || !data.length || allHidden) && !loading &&
      (getSlotsInRender(this,'empty') || <div class={`${prefix}-empty`}>{emptyText}</div>)
    } */
    ...
    >
      {((!data || !data.length || allHidden))
        ? (getSlotsInRender(this, 'empty') || <div class={`${prefix}-empty`} style={{
