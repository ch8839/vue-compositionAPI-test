...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const featPager = {
      currentPage, curPageSize, m_filteredData, filteredDataLen,
    }
  ...
  render() {
    const {
      ...
      placeholder, noMatchText, noDataText, filterable, hasNoMatch, panelData, checked, m_filteredData,
      disabledProp, labelProp, keyProp, filteredDataLen, pagination, direction,

    } = this

    const $render = getScopedSlotsInRender(this)
    const $renderChildren = getScopedSlotsInRender(this, 'children')

    const renderContent = () => {
      return $renderChildren
        && $renderChildren({
          filteredData: m_filteredData,
          direction: direction,
        ...
        >
          {m_filteredData.map((item: any) => <mtd-checkbox
            class={`${prefix}-item`}
