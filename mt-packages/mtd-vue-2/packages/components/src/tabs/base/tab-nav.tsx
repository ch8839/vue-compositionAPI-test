import { computed, defineComponent, PropType, hasProp, vSlots, getSlotsInRender } from '@ss/mtd-adapter'
...
import { ITabPane, ITabDrop, ITabBar, TabPosition, isTabDrop } from '../types'
...
export default defineComponent({
  ...
  methods: {
    ...
    computedActive(currentValue: any, tabItem: ITabPane | ITabDrop) {
      if (isTabDrop(tabItem)) {
        const tabDropItem = tabItem
        const { tabDropInfo } = tabDropItem

        if (tabDropInfo) {
          return tabDropInfo.moreTabs.some((it) => {
            return currentValue === it.value
          })
        }
      } else {
        return (tabItem).active = currentValue === (tabItem).value
      }
    },
    getNextStatus() {
  ...
  render() {
    ...
    const tabs = panes.map((item, index) => {
      ...
      const tabOneSlots = {
        label: getSlotsInRender(this, 'label') ? () => getSlotsInRender(this, 'label') : undefined,
      }

      const tabOneListeners = {
      ...
      return <tab-one
        size={size} disabled={disabled} active={active} icon={icon}
        label={label} pane={item} tabDropInfo={tabDropInfo} value={value}
        key={value} currentValue={this.currentValue}
