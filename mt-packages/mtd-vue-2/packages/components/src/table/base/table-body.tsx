...
export default defineComponent({
  ...
  setup(props) {
    ...
    function getVisibleColumns<T extends IColumn>(columns: T[]): T[] {
      if (isHorizontalVirtual.value) {
        return columns.filter(col => col.visible || col.fixed)
      } else {
        return columns
      }
    }
    ...
    const verticalVirtualProps = computed(() => {
      ...
      return useResetAttrs(temp).value
    })
  ...
  render() {
    ...
    const renderTreeExpaned = (
      ...
      !hasChild && classes.push(`${prefixTable}-hidden`)
    ...
    const slots = {
      headerViewTag: isHorizontalVirtual
        ? () => <div style={{ width: totalWidth + 'px', float: 'left', height: 0 }} />
        : undefined,
      headerViewListTag: renderColGroup,
