...
class TableLayout {
  ...
  updateColumnsWidth() {
    ...
    const getFixedMergedColWidth = (column: IColumn) => {
      function dfs(col: IColumn, sum: number): number {
        if (Array.isArray(col.children) && col.children.length > 0) {
          return col.children.reduce((total: number, m_col: IColumn) => total + dfs(m_col, sum), sum)
        } else {
          return col.realWidth || 0
        }
      }
      return dfs(column, 0)
    }
    ...
    if (fixedColumns.length > 0) {
      ...
      fixedColumns.forEach(function (column) {
        if (Array.isArray(column.children) && column.children.length > 0) {
          ...
          fixedWidth += column.realWidth || column.width || 0
        }
      })
