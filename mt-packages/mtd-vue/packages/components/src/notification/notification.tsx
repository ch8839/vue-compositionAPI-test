...
export default defineComponent({
  ...
  setup(props, { attrs }) {
    ...
    const positionStyle = computed<Object>(() => {
      return {
    ...
    function destroyElement() {
      if (el.value) {
        ...
        (props.destroyed as Function)() // 🤡 function props 怎么处理
      }
    }
  ...
  render() {
    ...
    return <transition name={`${prefix}-fade`}>
      ...
      >
        {typeIcon &&
          <i class={[`${prefix}-icon`, typeIcon]} />
        }
        <div class={`${prefix}-group`}>
          <div class={`${prefix}-title`} domPropsInnerText={title} />
          {displayTime &&
            <div class={`${prefix}-time`} domPropsInnerText={mountTime} />
          }
