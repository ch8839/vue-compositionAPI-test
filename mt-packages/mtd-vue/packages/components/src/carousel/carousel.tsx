...
function getCarouselItemVNodesFromSlots(vnodes: VNode[]) {
  return (vnodes || []).filter((vnode) => {
    if (vnode.componentOptions) {
      return (vnode.componentOptions.Ctor as any).options.name === 'MtdCarouselItem'
    }
    return false
  })
}
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const setActiveItem = (index: number, type?: 'next' | 'prev') => {
      ...
      if (isNormalType.value) {
        ...
        const curActiveItem = carouselInstanceItems.value[activeIndex.value] as any
        const nextActiveItem = carouselInstanceItems.value[index] as any
    ...
    // 获取 item ins
    const getCarouselItemInstanceList = (): CPI[] => {
      return ins.$children.filter((child: CPI) => child && child.$options.name === 'MtdCarouselItem')
    }

    // 获取 item vnode
    const getCarouselItemVNodeList = (): VNode[] => {
      const children = getSlots(ins) || []
      const result = getCarouselItemVNodesFromSlots(children)
      return result
    }
  ...
  render() {
    ...
    const vnodes = getCarouselItemVNodeList()
    const items = vnodes.map((i: VNode, index: number) =>
      applyProps(i, {
        index,
