...
import MtdCascaderMenu from './menu'
...
export default defineComponent({
  ...
  computed: {
    ...
    filteredNodes(): FilteredNode[] {
      // 搜索结果
      const { _fieldNames } = this
      return this.flattenNodes
        .filter((nodes: TreeNode[]) => {
          const paths = nodes.map((node) => node.data)
          return (this.filterMethod as Function)(this.filter, paths, _fieldNames)
        })
        .map((nodes) => {
      ...
      nodes.forEach((node) => {
        ...
        if (!hasChildren) {
          ...
          flatNodes = flatNodes.concat(
            this.getFlattenNodesInCascader(children as TreeNode[], stack),
          )
        }
      })
      return flatNodes
  ...
  render() {
    ...
    return <div
      class={classNames(this, {
        ...
        [`${prefix}-has-addendum`]: !!this.$slots['addendum-header'] || !!this.$slots['addendum-footer'],
      })}
      style={styles(this)}
    >
      {this.$slots['addendum-header'] && <div class={`${prefix}-addendum-header`}>
      ...
      <div class={prefix}>
        ...
        >
          {this.$slots.loading || <mtd-loading message={loadingText} />}
        </div >
      ...
      {this.$slots['addendum-footer'] && <div class={`${prefix}-addendum-footer`}>
