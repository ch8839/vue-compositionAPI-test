...
export default defineComponent({
  ...
  setup(props, { emit, slots }) {
    ...
    const updateTagsDomsWidth = () => {
      if (choicesMirror.value) {
        [...choicesMirror.value.children].forEach((tagDom, index) => {
          ...
          state.tagsValueWidthMap.set(tag[props.fieldNames.value], tagWidth)
        })
      }
    }
    ...
    const getRestTagWidth = (num: number) => {
      if (num) {
        ...
        return (state.restTagWidthMap as any)[digitNum] || 0
      }
      return 0
    }
    const reset = () => {
    ...
    return {
      ...
      handlePrefixClick, handleClose, reset,
      choices,
      mirror,
      choicesMirror,
      restTag,
  ...
  render() {
    const {
      ...
      resetTagsNum, fieldNames, mirrorListeners,
      status, hasFeedback, isNeedCalcRestTagWidth,
    } = this
    const { prefix: $prefix, suffix: $suffix } = $slots
    const iterator = (v: any, i: number) => {
      return <mtd-select-tag option={v} closable={closable}
        size={size} key={i} disabled={disabled || v.disabled}
        renderTag={renderTag}
        onClose={this.handleClose}
      >{formatter ? formatter(v) : v[fieldNames.label]}</mtd-select-tag>
    }
    return <div
      class={classNames(this, [
        `${inputPrefix}-wrapper`, `${prefixMTD}-select-multiple-input`, {
          [`${inputPrefix}-group`]: $slots.prepend || $slots.append,
    ...
    >
      ...
      <div class={inputPrefix} tabindex="0" onFocus={this.handleFocus}>
        ...
        <div class={`${prefixMTD}-multiple-select-choices`} ref="choices">
          ...
          {/* 搜索框 */}
          <mtd-multiple-mirror ref="mirror"
            prefix={prefix} shouldCollapse={collapseTags}
        ...
        <span class={`${inputPrefix}-suffix-inner`}>
          {loading
            ...
            : showClear
              ...
              />
              : ($suffix || <mtd-icon name={suffixIcon} />)
          }
