...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const text = computed(() => {
      if (props.texts) {
        // todo: 需要和设计确认是否是向上取整
        return props.texts[Math.ceil(props.modelValue) - 1] as string
      }
      return ''
    })
    ...
    const voidIconClass = computed(() => {
      const icon = props.disabled ? props.disabledVoidIcon : props.voidIcon
    ...
    const computedCollection = {
      rateClass, text, iconClasses, activeIconClass, decimalStyle,
    }
  ...
  render() {
    const {
      prefix, rateClass, disabled, currentValue, text, count, tooltipProps, texts,
    ...
    >
        ...
        {new Array(count).fill(0).map((num, index) => {
          ...
          >
            <template slot="content">
              {this.$slots.tooltip || (texts ? texts[item - 1] : '')}
            </template>
      ...
      {(text || $render) &&
        <span class={`${prefix}-text`}>
          {$render && $render({ value: currentValue }) || text}
        </span>
      }
