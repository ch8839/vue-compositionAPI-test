...
import { useAllAttrs, useAttrs, useClassStyle } from '@components/hooks/pass-through'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const panelOption = useAttrs(props.options)
    ...
    function formatDate(
      ...
      const formatter = (
        (props.multiple
          ...
          : TYPE_VALUE_RESOLVER_MAP[props.type as DatePickerType]) ||
    ...
    return {
      prefixCls: prefix, panelOption, onSelectionModeChange, classStyle, inputValueOptions, formatToModelValue,
  ...
  methods: {
    ...
    handleClickoutside() {
      if (this.readonly || this.disabled || this.holdOpen) {
        return
      }
      this.handleSubmit()
  ...
  date.setHours
    ...
    handlePickRange(days: Dayjs[]) {
      // todo: 确认是否要合并至 input 事件中
      this.$emit(
        'pick-range',
        days.map((day) => day.toDate()),
      )
    },
  ...
  render() {
    const {
      allAttrs, $slots, prefixCls, classStyle, inputValueOptions,
      _open: open, placement, disabled, appendToContainer, getPopupContainer, popperOptions,
    ...
    const InputComp = isRange(this) ? 'range-input' : 'date-picker-input'
    ...
    >
      ...
      <drop classProp={popperClass}>
        ...
        >
          {$slots.shortcuts && <template slot="shortcuts">
            {$slots.shortcuts}
          </template>}
          {/*  <template v-if="$slots.shortcuts" #shortcuts
          ><slot name="shortcuts"
        /></template>
        <template #cell="scope">
