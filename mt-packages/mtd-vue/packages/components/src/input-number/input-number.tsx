...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const getValidValueByMinAndMax = (val: number) =>
      Math.min(props.max, Math.max(props.min, val))
    watch(
      () => props.modelValue,
      (val) => {
        if (!isNumber(val)) {
    ...
    const handleInput = ({ target }: { target: HTMLInputElement }) => {
      let val = target.value.trim().replace(/。/g, '.')
    ...
    const setCurrentValue = (val: string | number | null) => {
      ...
      formItemHook._handleChange(val)
      userInput.value = null
    ...
    const handleUpStep = (e: KeyboardEvent) => {
      e.preventDefault()
      if (props.disabled || upHandleDisabled.value) return
      const val = userInput.value || currentValue.value
      const newVal = increase(val)
      setCurrentValue(newVal)
    }

    const handleDownStep = (e: KeyboardEvent) => {
      e.preventDefault()
      if (props.disabled || downHandleDisabled.value) return
      const val = userInput.value || currentValue.value
  ...
  render() {
    const { disabled, controls, controlsPosition, size, downHandleDisabled, upHandleDisabled,
      placeholder, displayValue, prefix, _status,
    } = this
    return <div
