import { computed, watch, ref } from "@ss/mtd-adapter"
import {
  FilteredNode,
  DisplayNode,
  TCheckedStrategy,
...
export const useDataHandler = (props: TreeProps, ctx: any, dataFieldName = 'data') => {
  ...
  function buildNodes(nodes: TreeNode[]): TreeNode[] {
    const expandedNodes = _expandedKeys.value
      .map((key) => {
    ...
    const checkedNodes = _checkedKeys.value
      .map((key) => {
        return nodeMap.get(key)!
      })
      .filter((node) => !!node)
  ...
  function setLoaded(node: TreeNode, data: TreeData, children: TreeData[]) {
    ...
    setLoadedKeys([..._loadedKeys.value, key])
    node.loaded = true
  }
  ...
  function setExpand(node: TreeNode, expanded: boolean) {
    const { key } = node
    const next = [..._expandedKeys.value]
    if (expanded) {
      next.push(key)
    } else {
      removeKey(key, next)
  ...
  function setChecked(node: TreeNode, checked: boolean) {
    ...
    if (props.checkStrictly) {
      ...
      checkedNodes = getCheckedNodes(root.value)
  ...
  /* 获取特定节点 */
  function getCheckedNodes(nodes: TreeNode[], ...args: any[]): TreeNode[] {
  ...
  function handleExpandedKeys(source: any[], target: any[]) {
    const childField = _fieldNames.value.children || 'children'
    source.forEach((item) => {
