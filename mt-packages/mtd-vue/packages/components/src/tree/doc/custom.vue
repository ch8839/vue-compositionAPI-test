<template>
  <div style="width:400px">
    <mtd-tree checkable :data="data" node-key="id"
      ...
      <template v-slot="{ node, data }">
        <div class="demo-tree-space-between" >
          ...
          <mtd-input v-else size="small"
            v-model="data.tempTitle" autofocus
            @blur="saveTitle(data)" />
          <span class="demo-tree-action" v-if="node.level > 0">
            <mtd-icon-button size="small"
              icon="edit-o" @click.stop="editNode(node, data)" /><mtd-icon-button
              size="small"
              icon="delete-o" @click.stop="removeNode(node, data)" />
          </span>
        </div>
      </template>
    </mtd-tree>
